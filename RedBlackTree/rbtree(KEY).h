/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Ankur Ramesh and Alex Vallorosi
 * Version     : 1.0
 * Date        : 4/19/2016
...
public:
    ...
    bool check_duplicates(Node<K, V> *root, Node<K, V> *search) {
        ...
        if (search->value() == root->value()) { return true; }
        return (check_duplicates(root->left(), search)
                or check_duplicates(root->right(), search));
    }
...
private:
    ...
    void left_rotate(Node<K, V> *x) {
        ...
        x->set_right(y->left());
        if (y->left() != NULL) { y->left()->set_parent(x); }
        y->set_parent(x->parent());

        if (x->parent() == NULL) { root_ = y; }
        else if (x == (x->parent()->left())) { x->parent()->set_left(y); }
        else { x->parent()->set_right(y); }
        y->set_left(x);
        x->set_parent(y);
    }
    ...
    void right_rotate(Node<K, V> *x) {
        ...
        x->set_left(y->right());
        if (y->right() != NULL) { y->right()->set_parent(x); }
        y->set_parent(x->parent());
        if (x->parent() == NULL) { root_ = y; }
        else if (x == (x->parent()->right())) { x->parent()->set_right(y); }
        else { x->parent()->set_left(y); }
        y->set_right(x);
        x->set_parent(y);
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        ...
        if (node != NULL && left_temp == NULL && right_temp == NULL) { return 0; }
        else if ((left_temp == NULL) && (right_temp != NULL)) { return 1 + internal_node_count(right_temp); }
        else if ((left_temp != NULL) && (right_temp == NULL)) { return 1 + internal_node_count(left_temp); }
        else { return (internal_node_count(node->left()) + internal_node_count(node->right())) + 1; }
    }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        ...
        else { return width(node->left(), level - 1) + width(node->right(), level - 1); }
    }
    ...
    size_t null_count(Node<K, V> *node) const {
        if (node == NULL) { return 1; }
        else { return null_count(node->left()) + null_count(node->right()); }
    }
